...
**Exemplo:**  
![Logo do RMarkdown](https://miro.medium.com/max/600/1*sCJzUnDilAuvGrlllJeXKw.jpeg){width=45%}
...
"Esse cálculo é realizado pela equação $\sum_{i = 1} (x_i - \mu)^2$ ou $\displaystyle{\sum_{i = 1} (x_i - \mu)^2}$ como comentado no item anterior."
...
```{r Carregamento de pacotes, message=FALSE}
library(dplyr)
library(rstatix)
```

Carregando o Banco de dados implícitamente
```{r carregamento do banco de dados, include=FALSE}
dados <- read.csv2("Banco_de_Dados_6.csv")
glimpse(dados)
```
  
Visualização do banco de dados (desformatado e apenas as 10 primeiras linhas de código)
